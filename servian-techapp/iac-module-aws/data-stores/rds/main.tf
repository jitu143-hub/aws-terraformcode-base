...
module "aws_cloudwatch_metric_alarm" {
  ...
  alarm_name                = "RDS-${var.deployment_env}-${var.identifier}-FreeStorageSpace"
  threshold                 = var.allocated_storage*1024*1024*1024*20/100
  identifier                = var.identifier
  aws_sns_arn               = var.aws_sns_arn
  create_metric_alarm       = var.create_alerts
}

#module "pagerduty_ruleset_rules" {
#  source = "./pagerduty_ruleset_rules"
#  identifier = var.identifier
#  subject = "RDS-${var.deployment_env}-${var.identifier}-FreeStorageSpace"
#  pagerduty_service_id      = var.pagerduty_service_id
#  create_ruleset_rules      = var.create_alerts
#  severity                  = var.deployment_env == "prod" ? "critical" : "warning"
#}